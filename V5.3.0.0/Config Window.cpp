...
LRESULT CALLBACK Page1Proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_PAINT:
		{
			...
			RectDrawText.bottom = RectDrawText.top + g_FontSize.cy * 1.3;
			DrawText(hdc, TEXT("窗口大小"), -1, &RectDrawText, DT_RIGHT | DT_SINGLELINE);

			RectDrawText.top = g_LineAnchorY[6] - g_FontSize.cy * 0.2;
			RectDrawText.bottom = RectDrawText.top + g_FontSize.cy * 1.3;
		...
		case WM_COMMAND:
		{
			...
			switch (LOWORD(wParam))
			{
				...
				case ID_BTN1_Execute:
				{
					...
					ShowCmd = SendMessageW(g_ComboHwnd1_CmdShow, CB_GETCURSEL, 0, 0);
					//组合框位置转SW_样式
					switch(ShowCmd)
					{
						default:
					...
					if (!CreateShortcut(OutputLnk, argv[0], Args_, InitPath, Path, 0, ShowCmd, TEXT("UACSkip白名单"), true, L"%UACSkip%"))
					{
...
LRESULT CALLBACK Page3Proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_PAINT:
		{
			...
			RectDrawText.bottom = RectDrawText.top + g_FontSize.cy * 1.3;
			DrawText(hdc, TEXT("版本：5.2.0.0                日期：2025.7.28"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		g_ComboHwnd1_CmdShow = CreateWindowEx(
			NULL,
			L"COMBOBOX",
			NULL,
		...
		SendMessage(g_ComboHwnd1_CmdShow, WM_SETFONT, (WPARAM)g_EditFont, 0);
		SendMessageW(g_ComboHwnd1_CmdShow, CB_ADDSTRING, 0, (LPARAM)L"正常窗口");
		SendMessageW(g_ComboHwnd1_CmdShow, CB_ADDSTRING, 0, (LPARAM)L"最小化");
		SendMessageW(g_ComboHwnd1_CmdShow, CB_ADDSTRING, 0, (LPARAM)L"最大化");
		...
		g_EditHwnd1_OutputLnk = CreateWindowEx(
			...
			WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
			g_FontSize.cx * 8.5,
			g_LineAnchorY[6] - g_FontSize.cy * 0.2,
			g_PageRect.right - g_PageRect.left - g_FontSize.cx * 8.5 - g_FontSize.cx * 5,
			g_FontSize.cy * 1.4,
			g_PageHwnd[0],
			NULL,
		...
		if (TaskInstalled)
		{
			...
			{
				SetWindowText(g_ButtonHwnd1_ContextMenu, TEXT("修复右键菜单"));
				POINT pt = { g_FontSize.cx * 6,g_LineAnchorY[7] + g_FontSize.cy };
				ClientToScreen(g_PageHwnd[0], &pt);
...
void Config()
{
	...
	SIZE WindowSize = AdjustClientSize(18, 10, 18, true, &g_ClientSize, WS_OVERLAPPED | WS_VISIBLE | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX, NULL, NULL);
	...
	ShowWindow(g_MainHwnd, g_iCmdShow == SW_MAXIMIZE ? SW_NORMAL : g_iCmdShow);
